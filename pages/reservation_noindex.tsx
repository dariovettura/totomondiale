...
export default function Home() {
  ...
  function disabledDate(current: any) {
    ...
    let disabledDate = []
    return disabledDate = current.filter((el: moment.MomentInput) => moment(el).day() !== 1)
    
  }

  const getCalendar= ()=>{
    return axios.get("/api/getCalendar").then(res=>{console.table(res.data)})
  }
  ...
  React.useEffect(() => {
   ...
   console.log( disabledDate(getDatesInRange(allDate[0].startDate, allDate[0].endDate)))
...
console.log(Calendar)
  return (
    <div >
      ...
      >
        {day.map((el, i) => <Option value={el.value} label={el.label} key={i}> {el.label}</Option>)}
      </Select>
      <Select
        mode="multiple"
        allowClear
      ...
      >
        {hours.map((el, i) => <Option value={el} label={el} key={i}>{el}</Option>)}
      </Select>
      <Select
        mode="multiple"
        allowClear
